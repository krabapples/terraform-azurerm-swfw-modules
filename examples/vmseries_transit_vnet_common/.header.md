...
**For Prisma AIRS deployments**, you must also accept the AIRS marketplace terms:

```bash
# Find available AIRS versions
az vm image list -o table --all --publisher paloaltonetworks --offer airs-flex

# Accept the AIRS marketplace terms (replace <version> with your chosen version)
az vm image accept-terms --publisher paloaltonetworks --offer airs-flex --plan airs-byol --subscription <your-subscription-id>
```
